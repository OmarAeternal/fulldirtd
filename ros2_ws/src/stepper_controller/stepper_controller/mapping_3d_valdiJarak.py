...
class Mapping3D(Node):
    ...
    def __init__(self):
        ...
        self.save_counter = 1
        self.save_root = Path.home() / "bags" / "akurasi_jarak" / "tes"
    ...
    def save_to_mcap(self, cloud):

        folder_name = f"{self.save_counter:03d}"

        bag_path = self.save_root / folder_name
        ...
        writer.write(
            "/map_3d",
            serialize_message(cloud),
            self.get_clock().now().nanoseconds
        )

        self.get_logger().info(
            f"Saved bag : {bag_path}"
        )
